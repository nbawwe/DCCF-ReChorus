...
class LGCNEncoder(nn.Module):
	def __init__(self, user_count, item_count, emb_size, norm_adj, n_intents,  n_layers=3):
	...
	def forward(self, users, items):
		# 全部嵌入 = 用户嵌入+物品嵌入 维度(user+item, emb_size)
		ego_embeddings = torch.cat([self.embedding_dict['user_emb'], self.embedding_dict['item_emb']], 0)
		all_embeddings = [ego_embeddings]		# 用于存储每一层的ego_embeddings
		# self.layers = emb_size * 实际层数
		for k in range(len(self.layers)):
			# 每一层z = AE， 包括了人和物
			ego_embeddings = torch.sparse.mm(self.sparse_norm_adj, ego_embeddings)
			all_embeddings += [ego_embeddings]
		# 聚合embeddings，先叠起来成3维再取均值
