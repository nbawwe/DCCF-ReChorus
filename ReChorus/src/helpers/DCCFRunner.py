...
def group_indices_by_user(dataset):
	"""
	扫描 Dataset，将用户 ID 映射到索引
	"""
	user_to_indices = {}
	for idx in range(len(dataset)):
		user_id = dataset[idx]['user_id']
...
class DynamicUserSampler(Sampler):
	def __init__(self, user_to_indices, num_batches, batch_size, samples_per_user):
		#super().__init__()
		self.user_to_indices = user_to_indices
		self.unique_user_ids = list(user_to_indices.keys())
		self.total_samples = num_batches * batch_size
		self.samples_per_user = samples_per_user

	def __iter__(self):
		batch_indices = []
		# 等权随机采样用户 ID，总计需要 `total_samples`
		for _ in range(self.total_samples):
			sampled_user_id = random.choice(self.unique_user_ids)
			user_indices = self.user_to_indices[sampled_user_id]
			# 从该用户的样本中随机选择
			batch_indices.append(random.choice(user_indices))
...
class DCCFRunner(object):
	...
	def fit(self, dataset: BaseModel.Dataset, epoch=-1) -> float:
		...
		model.train()
		loss_lst = list()
		# 移除了shuffle=true，加入了sampler
		user_to_indices = group_indices_by_user(dataset)
		sampler = DynamicUserSampler(user_to_indices, num_batches=40, batch_size=self.batch_size, samples_per_user=1)
		dl = DataLoader(dataset, collate_fn=dataset.collate_batch, sampler=sampler, batch_size=self.batch_size)
		# dl = DataLoader(dataset, batch_size=self.batch_size, num_workers=0,
		# 				collate_fn=dataset.collate_batch, pin_memory=self.pin_memory,
		# 				sampler=RandomSampler(dataset, replacement=True))


		for batch in tqdm(dl, leave=False, desc='Epoch {:<3}'.format(epoch), ncols=100, mininterval=1):
			batch = utils.batch_to_gpu(batch, model.device)		# 包括user_id, item_id, batch_size, phase
			# randomly shuffle the items to avoid models remembering the first item being the target
			item_ids = batch['item_id']	# 每个批正负样本
			...
			# prediction = out_dict['prediction']	# 对rank任务是(256, 2)
			# if len(prediction.shape)==2: # only for ranking tasks
			# 	# 创建一个和prediction格式一样的空张量(256,2)
