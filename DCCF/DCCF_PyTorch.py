...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    for epoch in range(args.epoch):
        ...
        n_samples = data_generator.uniform_sample()         # n_batch * batch_size  409600个(用户+正负样本)
        n_batch = int(np.ceil(n_samples / args.batch_size))     # batch_size = 10240, 重新计算回n_batch

        _model.train()
        loss, mf_loss, emb_loss, cen_loss, cl_loss = 0., 0., 0., 0., 0.
        for idx in tqdm(range(n_batch)):    # 分batch来
            # 清除上一轮梯度
            optimizer.zero_grad()

            # data_generator.mini_batch对总样本分割，这里按顺序分成40个batch
            ...
            # print(f"batch_mf_loss:{batch_mf_loss}, batch_cl_loss:{batch_cl_loss}")
